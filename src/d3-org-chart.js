...
export class OrgChart {
            ...
            buttonContent: ({ node, state }) => {
                const icons = {
                    "left": d => d ?
                      ...
                      </svg></span><span class="ychart-expand-btn-count" style="color:#716E7B">${node.data._directSubordinatesPaging} </span></div>` :
                        `<div class="ychart-expand-btn-content" style="display:flex;"><span class="ychart-expand-btn-icon" style="align-items:center;display:flex;"><svg width="8" height="8" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    "bottom": d => d ? `<div class="ychart-expand-btn-content" style="display:flex;"><span class="ychart-expand-btn-icon" style="align-items:center;display:flex;"><svg width="8" height="8" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                       ...
                       </svg></span><span class="ychart-expand-btn-count" style="margin-left:1px;color:#716E7B" >${node.data._directSubordinatesPaging} </span></div>
                       ` : `<div class="ychart-expand-btn-content" style="display:flex;"><span class="ychart-expand-btn-icon" style="align-items:center;display:flex;"><svg width="8" height="8" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            pagingButton: (d, i, arr, state) => {
                ...
                return `
                   <div class="ychart-paging-button" style="margin-top:90px;">
                      ...
                      </div><div class="ychart-paging-text" style="line-height:2"> Show next ${min}  nodes </div></div>
                   </div>
                `
            },
    ...
    render() {
        ...
        // Add keyboard handler for swap mode
        d3.select(window).on(`keydown.${attrs.id}`, (event) => {
            if (event.key === 's' || event.key === 'S') {
                console.log('S key pressed - swapMode:', attrs.swapMode, 'selected nodes:', attrs.selectedNodesForSwap.length);
                if (attrs.swapMode && attrs.selectedNodesForSwap.length === 2) {
                    console.log('Triggering performSwap');
                    this.performSwap();
                }
            }
        });
            ...
            attrs.svg
                ...
                .html(function (d, i, arr) {
                    if (d.data._pagingButton) {
                        return `<div class="ychart-paging-button-wrapper paging-button-wrapper" style="display:block;"><div class="ychart-paging-button-content" style="pointer-events:none">${attrs.pagingButton(d, i, arr, attrs)}</div></div>`;
                    }
    ...
    updateHtmlOverlay() {
        ...
        const overlayEnter = overlayNodes.enter()
            .append("div")
            .attr("class", "ychart-overlay-node overlay-node")
            .style("position", "absolute")
        ...
        overlayMerged.each(function(d, i, arr) {
            ...
            // Update content
            if (d.data._pagingButton) {
                contentWrapper.html(`<div class="ychart-paging-button-wrapper paging-button-wrapper" style="display:block;"><div class="ychart-paging-button-content" style="pointer-events:none">${attrs.pagingButton(d, i, arr, attrs)}</div></div>`);
            } else {
            ...
            if (hasChildren && !d.data._pagingButton) {
                if (buttonWrapper.empty()) {
                    buttonWrapper = container.append("div")
                        .attr("class", "ychart-overlay-button overlay-button")
                        .style("position", "absolute")
                        .style("cursor", "pointer")
                        .style("pointer-events", "auto");
                }
    ...
    enableSwapMode(enabled = true) {
        const attrs = this.getChartState();
        attrs.swapMode = enabled;
        if (!enabled) {
            // Clear selected nodes when disabling swap mode
            this.clearSwapSelection();
        }
        return this;
    }

    // Handle node click in swap mode
    handleSwapNodeClick(node) {
        const attrs = this.getChartState();
        
        // Check if node is already selected
        const index = attrs.selectedNodesForSwap.findIndex(n => attrs.nodeId(n.data) === attrs.nodeId(node.data));
        
        if (index !== -1) {
            // Deselect the node
            attrs.selectedNodesForSwap.splice(index, 1);
            node.data._selectedForSwap = false;
        } else {
            // Select the node (max 2 nodes)
            if (attrs.selectedNodesForSwap.length < 2) {
                attrs.selectedNodesForSwap.push(node);
                node.data._selectedForSwap = true;
            } else {
                // Replace the first selected node with the new one
                const firstNode = attrs.selectedNodesForSwap[0];
                firstNode.data._selectedForSwap = false;
                attrs.selectedNodesForSwap.shift();
                attrs.selectedNodesForSwap.push(node);
                node.data._selectedForSwap = true;
            }
        }
        ...
        // If two nodes are selected, check if they can be swapped
        if (attrs.selectedNodesForSwap.length === 2) {
            const [node1, node2] = attrs.selectedNodesForSwap;
            if (!this.canSwapNodes(node1, node2)) {
                console.log('Cannot swap nodes: they must be siblings (same parent and same level)');
            } else {
                console.log('Two nodes selected. Press "S" to swap or click to deselect.');
            }
        }
    }

    // Check if two nodes can be swapped (must be siblings at the same level)
    canSwapNodes(node1, node2) {
        if (!node1 || !node2) return false;
        
        // Check if nodes have the same parent and same depth
        const sameParent = node1.parent && node2.parent && 
                          node1.parent.data.id === node2.parent.data.id;
        const sameDepth = node1.depth === node2.depth;
    ...
    performSwap() {
        ...
        if (attrs.selectedNodesForSwap.length !== 2) {
            console.log('Please select exactly two nodes to swap');
            return this;
        }
        ...
        // Find the data objects in the data array
        const data1 = attrs.data.find(d => attrs.nodeId(d) === attrs.nodeId(node1.data));
        const data2 = attrs.data.find(d => attrs.nodeId(d) === attrs.nodeId(node2.data));
        
        if (!data1 || !data2) {
            console.log('Error: Could not find data for selected nodes');
            return this;
        }
        
        // Swap the parentId values to swap positions
        // We need to find their indices in the parent's children array
        const parent = node1.parent;
        const children = parent.children || parent._children || [];
        
        const index1 = children.findIndex(child => attrs.nodeId(child.data) === attrs.nodeId(node1.data));
        const index2 = children.findIndex(child => attrs.nodeId(child.data) === attrs.nodeId(node2.data));
        
        // Swap by modifying a custom sort property
        // If nodes don't have a sort order, assign one based on current position
        if (data1._sortOrder === undefined) {
            attrs.data.forEach((d, i) => {
                if (d._sortOrder === undefined) d._sortOrder = i;
            });
        }
        
        // Swap the sort order
        const tempOrder = data1._sortOrder;
        data1._sortOrder = data2._sortOrder;
        data2._sortOrder = tempOrder;
        
        // Sort the data array by parent and sortOrder
        attrs.data.sort((a, b) => {
            const parentA = String(attrs.parentNodeId(a) || '');
            const parentB = String(attrs.parentNodeId(b) || '');
    ...
    clearSwapSelection() {
        ...
        attrs.selectedNodesForSwap.forEach(node => {
            node.data._selectedForSwap = false;
        });
        
        attrs.selectedNodesForSwap = [];
    ...
    clear() {
        ...
        d3.select(window).on(`keydown.${attrs.id}`, null);
